import XCTest
@testable import AquariumLog

@MainActor
final class AquariumLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Fish(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(Fish(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Fish(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(Fish(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = Fish(title: "Delete Me")
        _ = store.add(item, isPro: false)
