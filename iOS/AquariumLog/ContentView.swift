...
struct ContentView: View {
    ...
    @State private var editingItem: Fish?
    ...
    @ViewBuilder
    func row(for item: Fish) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(item.title)
                .font(Theme.headlineFont)
                .foregroundStyle(Theme.ink)
            Text("\(String(describing: item.species))  \u{00B7}  \(String(describing: item.tank))")
                .font(.caption)
...
struct ItemEditorView: View {
    ...
    let mode: EditorMode
    let onSave: (Fish) -> Void

    @State private var draft: Fish

    init(mode: EditorMode, onSave: @escaping (Fish) -> Void) {
        self.mode = mode
        self.onSave = onSave
        switch mode {
        case .add:
            _draft = State(initialValue: Fish(title: ""))
        case .edit(let item):
        ...
        NavigationStack {
            Form {
                Section("Fish") {
                    ...
                    TextField("Tank", text: $draft.tank)
                        .accessibilityIdentifier("field.tank")
            ...
            .navigationTitle(mode == .add ? "Add Fish" : "Edit Fish")
            .toolbar {
